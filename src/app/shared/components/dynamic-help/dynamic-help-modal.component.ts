...
import { HelpService, HelpDTO } from 'app/core/services/help.service';
...
export class DynamicHelpModalComponent implements OnInit {
    ...
    saveHelp(): void {
        this.isLoading = true;
        const helpToSave: HelpDTO = {
            id: this.currentHelp?.id,
            path: this.path,
            content: this.editContent
        };

        this._helpService.saveHelp(helpToSave)
            .pipe(finalize(() => this.isLoading = false))
            .subscribe({
                next: (success) => {
                    if (success) {
